...
class Combo {
    ...
    public function montaComboCategoria2(){
    ...
    public function montaComboSubcategoriaSelecionado($id){
        
        $busca = $this->ci->subcategoria_model->get_all();
        

        $combo="<select name='subcategorias' id='subcategorias'>";
        $combo.="<option value=''>Selecione uma Categoria</option>";

        foreach ($busca->result() as $linha) {
            if($linha->id == $id){
            $combo.="<option value='{$linha->id}'selected>{$linha->nome}</option>";
            }else{
