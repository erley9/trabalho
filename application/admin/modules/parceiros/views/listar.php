...
<body>
	...
	<div id="container">
		...
		<section id="centro">
			...
			<h1>Lista de Parceiros Cadastrados</h1>
			<a href="parceiros/cadastrar" class="cadastrar">Cadastrar</a>
			<table id="tbl-clientes">
				...
				<tbody id="tabela">
					...
					foreach ($lista->result() as $linha) {
						...
						<tr>
							<td>{$linha->nome}</td>
							<td align='center'><img src='{$this->base->base_adm("img/parceiros/{$linha->foto}")}' alt='' class='logo'></td>
							<td align='center'><a href=http://'{$linha->url}'>{$linha->url}</a></td>
							<td align='center'><a href='".base_url("parceiros/editar/{$linha->id}")."'><img src='{$this->base->base_adm("img/adm/editar-icon.png")}' alt=''></a></td>
							<td align='center'><a href='".  base_url("parceiros/excluir/{$linha->id}")."'><img src='{$this->base->base_adm("img/adm/excluir-icon.png")}'' alt=''></a></td>
						</tr>";
					}
